...
const WaterLevel = ({ db_rt, db, username, userInfo }) => {
  ...
  useEffect(() => {
    ...
    const waterLevelRef = ref(db_rt, "user2/water_level/current");
  ...
  useEffect(() => {
    ...
    const auto1Ref = ref(db_rt, "user2/auto2");
  ...
  useEffect(() => {
    ...
    const pump1Ref = ref(db_rt, "user2/pump2");
  ...
  useEffect(() => {
    ...
    const valve1Ref = ref(db_rt, "user2/valve2");
    let isFirstLoad = true;
  ...
  const startSystem = async () => {
    ...
    try {
      ...
      await controlDevice("user2/valve2", "on");

      console.log("⏳ Đợi 3 giây...");
      await delay(5);

      console.log("💧 Bật máy bơm (pump1)...");
      await controlDevice("user2/pump2", "on");
  ...
  const stopSystem = async () => {
    ...
    try {
      ...
      await controlDevice("user2/pump2", "off");

      console.log("⏳ Đợi 3 giây...");
      await delay(3);

      console.log("🔂 Đóng van (valve1)...");
      await controlDevice("user2/valve2", "off");
